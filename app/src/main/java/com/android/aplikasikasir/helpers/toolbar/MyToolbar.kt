package com.android.aplikasikasir.helpers.toolbar

import android.content.Context
import android.util.AttributeSet
import android.view.LayoutInflater
import android.view.View
import android.widget.FrameLayout
import android.widget.TextView
import androidx.appcompat.widget.Toolbar
import com.android.aplikasikasir.R

class MyToolbar: Toolbar {

    interface NavigateUpListener {
        fun onNavigationUpClicked()
    }

    private var navigateUpListener: () -> Unit = {}
    private var navigateHistoryListener: () -> Unit = {}
    private var navigatePreviewListener: () -> Unit = {}

    private lateinit var navigateUp: FrameLayout
    private lateinit var navigateHistory: FrameLayout
    private lateinit var txtTitle:TextView


    constructor(context: Context) : super(context) {
        init(context)
    }

    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs) {
        init(context)
    }

    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs, defStyleAttr) {
        init(context)
    }

    private fun init(context: Context) {
        val view = LayoutInflater.from(context).inflate(R.layout.layout_my_toolbar, this, true)
        setContentInsetsRelative(0, 0)
        navigateUp = view.findViewById(R.id.navigate_up)
        navigateHistory = view.findViewById(R.id.navigate_history)
        txtTitle = view.findViewById(R.id.txtTitle)
        navigateUp.setOnClickListener { navigateUpListener.invoke() }
        navigateHistory.setOnClickListener { navigateHistoryListener.invoke() }
    }

    fun setNavigateUpListener(navigateUpListener: () -> Unit) {
        this.navigateUpListener = navigateUpListener
        navigateUp.visibility = View.VISIBLE
    }

    fun setNavigateHistoryListener(navigateHistoryListener: () -> Unit) {
        this.navigateHistoryListener = navigateHistoryListener
        navigateHistory.visibility = View.VISIBLE
    }

}